...
public final class JispyInterpreter {
    ...
    private void initGlobalEnv() {
        ...
        GlobalEnv.put(new Symbol("eval"), (Lambda) args -> {
            if (args.size() != 1) {
                throw new Exceptions.ArgumentsCountException();
            }
    ...
    private static @NotNull Object readAhead(@NotNull Object token, @NotNull InputPort inPort) {
        if (token.equals("(")) {
            ...
            throw new Exceptions.SyntaxException("unexpected )");
        } else if (quotes.containsKey(token)) {
            return treeList(quotes.get(token), read(inPort));
        } else if (token.equals(eof)) {
            throw new Exceptions.SyntaxException("unexpected EOF in list");
        } else {
    ...
    static String evalToString(Object x) {
        if (x == null) {
            ...
            return "#f";
        } else if (x instanceof Symbol symX) {
            return symX.Value();
        } else if (x instanceof String strX) {
            return StringEscapeUtils.unescapeJava(strX);
            ...
            s.append(")");
            return s.toString();
        } else if (x instanceof Complex complexX) {
            return ComplexFormat.getInstance().format(complexX);
        } else {
            return x.toString();
        }
    }
    ...
    private Object expand(Object x, boolean topLevel) {
        ...
        if (op.equals(_quote)) {
            ...
            if (l.size() == 1) {
                ...
                return l.stream().map(i -> expand(i, topLevel)).collect(Collectors.toList());
            }
        } else if (op.equals(_lambda)) {
            require(x, l.size() >= 3);
            var vars = l.get(1);
